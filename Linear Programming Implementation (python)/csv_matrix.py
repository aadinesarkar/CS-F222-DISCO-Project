...
def csvToMatrix(file_path, courses_file_path):
    # Read CSV file into a DataFrame
    df = pd.read_csv(file_path)
    courses = pd.read_csv(courses_file_path)
    ...
    #     df['FD CDC'].isin(courses['Course code']), pd.NA)
    # df['HD CDC'] = df['HD CDC'].where(
    #     df['HD CDC'].isin(courses['Course code']), pd.NA)
    # df['FD Elec'] = df['FD Elec'].where(
    #     df['FD Elec'].isin(courses['Course code']), pd.NA)
    # df['HD Elec'] = df['HD Elec'].where(
    #     df['HD Elec'].isin(courses['Course code']), pd.NA)
    # ####

    fd_cdc_courses_exist = df['FD CDC'].isin(
        courses.loc[courses['Type'] == 'FD_CDC', 'Course code'])
    df.loc[~fd_cdc_courses_exist, 'FD CDC'] = np.NaN
    hd_cdc_courses_exist = df['HD CDC'].isin(
        courses.loc[courses['Type'] == 'HD_CDC', 'Course code'])
    df.loc[~hd_cdc_courses_exist, 'HD CDC'] = np.NaN
    fd_elec_courses_exist = df['FD Elec'].isin(
        courses.loc[courses['Type'] == 'FD_Elec', 'Course code'])
    df.loc[~fd_elec_courses_exist, 'FD CDC'] = np.NaN
    hd_elec_courses_exist = df['HD Elec'].isin(
        courses.loc[courses['Type'] == 'HD_Elec', 'Course code'])
    df.loc[~hd_elec_courses_exist, 'HD Elec'] = np.NaN

    # read the CDC's from the Courses
    # Get unique values of Course code where Type is FD_CDC
    unique_fd_cdc = courses.loc[courses['Type']
                                == 'FD_CDC', 'Course code'].unique()
    unique_hd_cdc = courses.loc[courses['Type']
                                == 'HD_CDC', 'Course code'].unique()

    # Calculate the total number of unique courses
    # total_courses = (df['HD Elec'].nunique() + df['FD Elec'].nunique() + df['HD CDC'].nunique() + df['FD CDC'].nunique())
    ...
    filtered_courses_df = courses[courses['Course code'].isin(unique_courses)]
    ####
    ...
    while True:
        ...
        unique_courses = set(unique_fd_cdc) | set(
            unique_fd_elec) | set(unique_hd_cdc) | set(unique_hd_elec)
        # Create a new DataFrame containing only the relevant courses and their sections
        filtered_courses_df = courses[courses['Course code'].isin(
            unique_courses)]
        total_sections_filtered = filtered_courses_df['Sections'].sum()
    ...
    # Group by 'Name' and aggregate using the modified custom function
    merged_df = df.groupby('Name').agg(custom_agg).reset_index()

    # Check for missing FD_CDC courses and add columns with heading "FD CDC_{course code}"
    unique_fd_cdc = courses.loc[courses['Type']
                                == 'FD_CDC', 'Course code'].unique()
    for course_code in unique_fd_cdc:
        column_name = f"FD CDC_{course_code}"
        if column_name not in merged_df.columns:
            merged_df[column_name] = 0

    # Check for missing HD_CDC courses and add columns with heading "HD CDC_{course code}"
    unique_hd_cdc = courses.loc[courses['Type']
                                == 'HD_CDC', 'Course code'].unique()
    for course_code in unique_hd_cdc:
        column_name = f"HD CDC_{course_code}"
        if column_name not in merged_df.columns:
            merged_df[column_name] = 0
...
def processMergedDF(merged_df, filtered_courses_df):
    ...
    for header in headers_vector:
        ...
        sections = filtered_courses_df.loc[filtered_courses_df['Course code']
